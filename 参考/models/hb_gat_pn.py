...
class HBGATPN(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        # Global Context: 对 Station 节点进行 Mean Pooling
        # Station 节点通常聚合了全局信息
        if hasattr(batch_data['station'], 'batch') and batch_data['station'].batch is not None:
             batch_index = batch_data['station'].batch
             global_context = global_mean_pool(x_dict_encoded['station'], batch_index) # [B, H]
        else:
             global_context = torch.mean(x_dict_encoded['station'], dim=0, keepdim=True)
